...
public class Junction extends SimulatedObject {
	...
	//Devuelve la lista de carreteras entrantes
	public List<Road> getInRoads() {
		return Collections.unmodifiableList(new ArrayList<>(this.inRoads));
	}

	//Devuelve que carretera tiene el semáforo en verde
	public int getGreenLightIndex() {
		return this.light;
	}
	
	public String getRoadGreenLightIndex() {
		String s = "NONE";
		
		if(this.light != -1) {
			s = this.inRoads.get(this.light).getId();
		}
	...
	public String getQueues() {
		...
		if(this.inRoads.size() > 0) {
			Iterator <Road> it = inRoads.iterator();
			
			while(it.hasNext()) {
				Road r = it.next();
				s = s + r.getId() + ": [";
				
				List<Vehicle> vList = this.mapQueuesInRoads.get(r);
				Iterator <Vehicle> it2 = vList.iterator();
