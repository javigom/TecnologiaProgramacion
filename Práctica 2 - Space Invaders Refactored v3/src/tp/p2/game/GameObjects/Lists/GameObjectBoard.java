...
public class GameObjectBoard {
	...
	//private GameObject ovni; - no necesario
	...
	private void removeDead() {
		...
		while(i < this.currentObjects) {
			...
			if(i != 0 && (objects[i].isOut() || !objects[i].isAlive())) {
				objects[i].onDelete();
				remove(objects[i]);
			}
			
			//else if(objects[i] == ovni && !objects[i].isAlive()) { //NUNCA elimina al ovni de la lista
			else if(i == 0 && !objects[i].isAlive()) { //NUNCA elimina al ovni de la lista
				objects[i].onDelete();
				i++;
			}
	...
	*/
	...
	//Realiza daño a los objectos que rodean la coordenada r, c
	public void explosion(int r, int c) {
		int pos = -1;
		for(int i = 0; i < 8; i++) {
			
			switch(i) {
				case 0: pos = getIndex(r - 1, c - 1);
					break;
				case 1: pos = getIndex(r - 1, c);
					break;
				case 2: pos = getIndex(r - 1, c + 1);
					break;
				case 3: pos = getIndex(r, c + 1);
					break;
				case 4: pos = getIndex(r + 1, c + 1);
					break;
				case 5: pos = getIndex(r + 1, c);
					break;
				case 6: pos = getIndex(r + 1, c - 1);
					break;
				case 7: pos = getIndex(r, c - 1);
					break;
			}
