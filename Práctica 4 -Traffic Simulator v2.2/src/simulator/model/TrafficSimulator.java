...
public class TrafficSimulator implements Observable<TrafficSimObserver> {
	...
	public void deleteEvent(Event e) throws Exception {
		...
		while(!deleted && i < this.eventList.size()) {
			
			if(e.equals(this.eventList.get(i))){
				this.deletedEventList.add(this.eventList.get(i));				
		...
		if(!deleted) {
			throw new Exception("The event you are trying to delete does not exist.");
		}
		
		else {
			this.notifyAdvanceEnd();
		}
	}
	
	public JSONObject saveStatusSim() {
		
		List<Event> events = new SortedArrayList<>();
		events.addAll(this.copyInitialEventList);
		events.addAll(this.copyNewEventList);
		events.removeAll(this.deletedEventList);
		...
		for(Event e: events) {
			jsonEvents.put(e.saveStatus());
		}
		
		info.put("events", jsonEvents);
		return info;
	}
